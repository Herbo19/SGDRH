...
<div class="col-sm-6">
    ...
    <form method="post" action="{{ route('meta.estado.atualizar',$editMeta->idMeta) }}">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="estado_meta">
                ...
                @foreach($editEstado as $editEstad)
                    ...
                    <option @if($editEstad->idEstadoMeta==$editMeta->idEstadoMeta) selected @endif value="{{$editEstad->idEstadoMeta}}">{{$editEstad->estado_meta}}</option>
        ...
        <a href="{{route('metas.show',$editMeta->idMeta)}}" class="btn btn-primary float-right">Ver</a>
    </form>
</div>
